...
def mg_otc(response):
    ...
    mrp1 = response.css('.PriceBoxPlanOption__offer-price-cp___2QPU_::text').extract()
    mrp2 = str(response.css('.DrugPriceBox__best-price___32JXw.selectorgadget_selected::text').extract_first())
    manufacturer = str(response.css('.ProductTitle__manufacturer___sTfon a').css('::text').extract_first())
    product_highlight = response.css('.ProductHighlights__product-highlights___2jAF5::text').extract()
    product_information = response.css('.ProductDescription__description-content___A_qCZ::text').extract()
    details.append({
        'name' : name,
        'mrp1' : mrp1,
        'mrp2' :mrp2,
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        google_link.append('https://www.google.com/search?q='+query+'+1mg&gbv=1&sei=FFM0Ys6xIKze2roP_MqnkAw')
